...
When a playlist generation request arrives, the data engine executes a seven-step retrieval-augmented generation workflow. The event description is first expanded by the HyDE provider [2] into a synthetic song description in order to bridge the semantic gap between short natural-language phrases and the richer embedding space of the indexed songs. The expanded query is used to retrieve the top candidate songs from the vector store, subject to a `max_distance` quality gate (production value: 0.80, calibrated by the eval harness); each retrieved candidate carries its persisted `vibe_tags`, `energy_desc`, and `mood_desc` metadata, which is fed into the DJ agent's generation prompt context. A relative margin filter then identifies a "strong spine" of closely matching songs, dynamic wildcard targets are computed from the remaining playlist slots, and the LangGraph DJ agent generates, validates, and — if necessary — retries wildcard suggestions before merging them with the spine songs in the final shuffled playlist. Validation is tri-state (valid, invalid, or transient error): a validator error is retried rather than treated as a rejected suggestion, and if every candidate in a round errors out the request fails loudly rather than returning a degraded playlist. The design and parameter choices for this pipeline are detailed in Section 3.3.
